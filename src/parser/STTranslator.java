...
public class STTranslator {
            ...
            if (term instanceof ParsedFunction){
                ...
                if (rule.head.equals("<=")) {
                    ...
                    if (rule.parameters.length > 0)
                        printTerm(rule.parameters[0], out);
                    out.print(" :- ");
                    if (rule.parameters.length > 1)
                        printTerm(rule.parameters[1], out);
                    for (int i = 2; i < rule.parameters.length; i++) {
                        out.print(", ");
                        printTerm(rule.parameters[i], out);
                    }
                } else
                    // rule is a fact
	...
	protected String replaceSpecialChars(String in) {
		return in.replace("_", "unterstrich_").replace("-", "minus_").replace("+", "plus_").replace("[", "eckigeklammerauf_").replace("]", "eckigeklammerzu_").replace("{", "geschweifteklammerauf_").replace("}", "geschweifteklammerzu_").replace(">", "spitzeklammerzu_").replace("*", "stern_").replace("/", "slash_").replace("\\", "backslash_").replace("&", "und_").replace("^", "dach_").replace("~", "tilde_").replace("'", "hochkomma_").replace("`", "hochkomma2_").replace("\"", "anfuehrungszeichen_").replace("@", "at_").replace("#", "raute_").replace("$", "dollar_").replace("%", "prozent_").replace(":", "doppelpunkt_").replace(",", "komma_").replace(".", "punkt_").replace("!", "ausrufezeichen_").replace("|", "strich_").replace("<", "spitzeklammerauf_").replace("=", "gleich_").replace("(", "klammerauf_").replace(")", "klammerzu_").replace(";", "semikolon_").replace("?", "fragezeichen_");
	}

	// rereplaces the special characters in a string
	public static String resetSpecialChars(String in) {
		return in.replace("minus_", "-").replace("plus_", "+").replace("unterstrich_", "_").replace("eckigeklammerauf_", "[").replace("eckigeklammerzu_", "]").replace("geschweifteklammerauf_", "{").replace("geschweifteklammerzu_", "}").replace("spitzeklammerzu_", ">").replace("stern_", "*").replace("slash_", "/").replace("backslash_", "\\").replace("und_", "&").replace("dach_", "^").replace("tilde_", "~").replace("hochkomma_", "'").replace("hochkomma2_", "`").replace("anfuehrungszeichen_", "\"").replace("at_", "@").replace("raute_", "#").replace("dollar_", "$").replace("prozent_", "%").replace("doppelpunkt_", ":").replace("komma_", ",").replace("punkt_", ".").replace("ausrufezeichen_", "!").replace("strich_", "|").replace("spitzeklammerauf_", "<").replace("gleich_", "=").replace("klammerauf_", "(").replace("klammerzu_", ")").replace("semikolon_", ";").replace("fragezeichen_", "?");
	}
    ...
    protected static class ParsedVariable extends ParsedObject {
        ...
        private ParsedVariable(String name) {
            super(name);
        }
    ...
    protected static class ParsedFunction extends ParsedTerm {
        ...
        public final ParsedTerm[] parameters;
        
        // should be constructed by calling parse, hence private
        private ParsedFunction(String head, ParsedTerm[] parameters) {
            this.head = head;
        ...
        public static ParsedFunction parse(StreamTokenizer tokenizer) throws IOException, ParseException {
            ...
            if (debug)
                System.out.println("ParsedFunction.parse(): "+tokenizer);
            return new ParsedFunction(head, parameterList.toArray(new ParsedTerm[]{}));
        }
        ...
        public int compare(ParsedTerm term1, ParsedTerm term2) {
            ...
            if (term1 instanceof ParsedObject)
                ...
                if ("<=".equals(function.head)) {
                    ParsedTerm parameter = function.parameters[0];
    ...
    public static String printKIF(Object object) {
		...
		if(object == null)
			...
			functor = STTranslator.resetSpecialChars(functor);
