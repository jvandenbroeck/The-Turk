...
def to_prolog(statement)
    if statement.class == Array
        ...
        #elsif head == "++"
		# return prolog + varToString(statement[2]) + " is " + varToString(statement[1]) + " + 1"
		# return prolog + "plusplus" + to_prolog(statement[1])
 	#	 prolog = "plus"
        ...
        #    return prolog.chop.chop + ")"
            ...
            prolog = "#{headToValid(statement[0])}("
            statement[1..(statement.size-1)].each do |term|
                prolog += "#{to_prolog(term)}, "
            end
            return prolog.chop.chop + ")"
...
def headToValid(head)
result = head.gsub(/\+/, "plus")
result = result.gsub(/-/, "min") 
end

def varToString(statement)
	if plusvar?(statement) 
            	return "P" + statement[1..statement.size].capitalize.chop.chop
	elsif minvar?(statement)
		return "M" + statement[1..statement.size].capitalize.chop.chop 
	else	
