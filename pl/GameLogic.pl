...
chance :- random_element(0 .. 9, X),!, ((X > 6) -> false; true). % 3/10 chance false, we don't update new value
...
randomMoves(CurrentState, RandomMoves) :-
	...
	(
		...
		(
			legalMoves(CurrentState, Role, LegalMoves),
                        ((qtree) -> maxQMove(CurrentState, LegalMoves, RandomMove, Role) ; random_element(LegalMoves, RandomMove))

		)
	).
randomGameWrite(CurrentState, TerminalState, MovesTemp, Moves) :-
...
randomGame(CurrentState, TerminalState, MovesTemp, Moves) :-
	(
		...
		(
			...
			nextState(CurrentState, RandomMoves, NextState),
			append(MovesTemp, [RandomMoves], MovesTemp2),
			randomGame(NextState, TerminalState, MovesTemp2, Moves)
		)
	).
% ggp_goal(oplayer, 100, S)
